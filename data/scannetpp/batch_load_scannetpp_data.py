...
# from .load_scannetpp_data import export
from load_scannetpp_data import export
...
def export_one_scan(scan_name,
                    ...
                    scannet_dir,
                    test_mode=False):
    mesh_file = osp.join(scannet_dir, scan_name, 'scans', 'mesh_aligned_0.05.ply')
    agg_file = osp.join(scannet_dir,
                        scan_name, 'scans', 'segments_anno.json')
    seg_file = osp.join(scannet_dir,
                        scan_name, 'scans', 'segments.json')
    print(mesh_file, agg_file, seg_file)
    if not osp.exists(mesh_file):
        print(f'No mesh file found: {mesh_file}')
        return
    if not osp.exists(agg_file):
        print(f'No annotation file found: {agg_file}')
        return
    if not osp.exists(seg_file):
        print(f'No segmentation file found: {seg_file}')
        return
